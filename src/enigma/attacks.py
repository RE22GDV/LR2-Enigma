...
def avalanche_test(
    ...
    """
    Міжсимвольна дифузія: яка частка символів шифротексту змінюється,
    якщо змінити ОДИН символ відкритого тексту.

    Для цієї конструкції змінюється рівно один символ, тобто ``1/L``:
    кожна позиція шифрується незалежно від решти.

    Увага: це НЕ строгий лавинний критерій — той сформульований для
    бітів (інверсія одного вхідного біта має змінювати кожен вихідний
    біт із імовірністю 1/2). Тут вимірюються символи, тому пряме
    порівняння отриманої частки з 50 % було б некоректним.
    """
    if not isinstance(rng, random.Random):
        rng = random.Random(rng)
